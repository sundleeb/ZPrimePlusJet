...
def bias(base,alt,ntoys,mu,iLabel):
    ...
    print "Random Seed = " + str(randseed)
    os.system('combine -M GenerateOnly     %s --rMax 75 --rMin -25 --toysFrequentist -t %i --seed %i --expectSignal %i --saveToys ' % (alt,ntoys, randseed, mu))
    os.system('combine -M MaxLikelihoodFit %s --rMax 75 --rMin -25 -t %i --saveNLL --toysFile higgsCombineTest.GenerateOnly.mH120.%s.root' % (base,ntoys,randseed))
    os.system('rm  higgsCombineTest.MaxLikelihoodFit.mH120.'+str(randseed)+'.root')
    os.system('mv  mlfit.root toys.root')
    plotgaus("toys.root",mu,"pull_"+str(randseed)+"_"+iLabel)
...
if __name__ == "__main__":
	options = parser()
	print options
	#plotgaus("toys.root",25.,"pull10GeVBIG")
