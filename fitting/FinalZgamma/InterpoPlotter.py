...
if __name__ == '__main__':
	print "-----------------------------------------------"
	BlankP = TH1F("BLANKP", ";AK8 Soft Drop Mass (GeV);a.u.", 50, 0, 200)
	BlankF = TH1F("BLANKF", ";AK8 Soft Drop Mass (GeV);a.u.", 50, 0, 200)
	BlankP.SetStats(0)
	BlankF.SetStats(0)
	Phists = []
	Fhists = []
	colors = []
	FTrue = TFile("PreProc/DATASETS/Zg_DATA1001.root")
	PhistsReal = []
	FhistsReal = []
	for m in [10,25,50,75,100,125]:
		print m
		pT = FTrue.Get("zqq"+str(m)+"_pass_msd")
		fT = FTrue.Get("zqq"+str(m)+"_fail_msd")
		pT.SetLineColor(1)
		pT.SetLineWidth(2)
		fT.SetLineColor(1)
		fT.SetLineWidth(2)
