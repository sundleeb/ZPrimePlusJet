...
def createHist(which, ddt,tag,filename, treename, sf,lumi,mass=0,isdata=False, issignal=False):
	...
	if which == "tt":
		...
		masslo   = 0;
		masshi   = 200;
		binBoundaries = [200, 232, 258, 294, 364, 1000]
		rbinBoundaries = [-7.5,-7.,-6.5,-6.,-5.5,-5.,-4.5,-4.,-3.5,-3.,-2.5,-2.,]
...
def FillHists(name, FDDT, FRAC, W, OPT):
	...
	#OLD: xs = [0.6927, 0.5254, 0.3312, 0.2868, 0.2473, 0.2345]
	xs = [2.504,  2.310, 2.127, 2.019, 1.914, 1.918]
...
for M in ["DATA"]:#, "MCETA"]:
	for b in ["100"]:
		...
		#	DDT25 = ["DDTs25_"+b+"_"+w+".root", "Smooth_N2DDT_"+M]
			FillHists("ZGp10", DDT10, 15,"sr",'gjets')
		#and if 	FillHists("ZGf10", DDT10, 1,"sr",'gjets')
