...
def main(options,args):
	...
	if options.np==2 and options.nr==2:
		dctpl = open("datacardtt.tpl");
	if options.np==3 and options.nr==2:
                dctpl = open("datacardtt32.tpl");
	if options.np==2 and options.nr==3:
                dctpl = open("datacardtt23.tpl");
        if options.np==3 and options.nr==3:
                dctpl = open("datacardtt33.tpl");
        if options.np==3 and options.nr==4:
                dctpl = open("datacardtt34.tpl");
        if options.np==4 and options.nr==3:
                dctpl = open("datacardtt43.tpl");
...
if __name__ == '__main__':
	...
	parser.add_option('-o','--odir', dest='odir', default = 'plots/',help='directory to write plots', metavar='odir')
        ...
        parser.add_option('--nmass', dest="nmass", type=int,default=50, help='number of mass bins')
