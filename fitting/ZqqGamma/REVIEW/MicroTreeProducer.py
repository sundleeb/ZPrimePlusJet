	...
	def FillMicroTree(self):
		...
		for j in range(0, self.n): # Here is where we loop over all events.
			...
			if  T.vpho0_mva > 0.2 and T.vpho0_pt > 200 and T.AK8Puppijet0_pt > 200. and T.AK8Puppijet0_msd > 0.:
				if not math.fabs(T.vpho0_eta < 2.4):
					eta5 = eta5 + 1
					if  not self.scaleByMVA: continue
				else: eta4 = eta4 + 1
		...
		if eta5 > 0:
			print "Eta renomalization = " + " ("+str(100*eta4/(eta5+eta4))+"%) events in SB"
...
if __name__ == '__main__':
	...
	#V200 = MicroTree("ZQQpG_200", "/eos/uscms/store/user/rapte/baconbits/VectorDiJet1Gammamva_M200_noeta.root", False, True)
