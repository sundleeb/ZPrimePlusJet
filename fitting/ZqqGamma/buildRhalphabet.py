...
class dazsleRhalphabetBuilder: 
	...
	def __init__( self, hpass, hfail, inputfile, in_np, in_nr ): 
		...
		self._outputName = "base"+str(in_np)+str(in_nr)+".root";
	...
	def RhoSafetyCheck(self, H, ptbin):
		for i0 in range(1,self._mass_nbins+1):
			imin = H.GetXaxis().GetBinLowEdge(i0)
			imax = H.GetXaxis().GetBinUpEdge(i0)
			r = [-2.0,-7.0]
			if ptbin == 1:
				p = [200.,230.]
				if imax > p[1]*math.exp(r[0]/2.):
	...
	def makeRhalph(self,iHs,iHPs,iPt,iCat):
		...
		self._lEffQCD.setVal(0.05)# @ MARC LOOK HERE!! HERE!!!!
	...
	def makeWorkspace(self,iOutput,iDatas,iFuncs,iVars,iCat="cat0",iShift=True):
		...
		sigMassesForInterpolation_25to75 = [];
		shapeForInterpolation_central_25to75 = [];
		shapeForInterpolation_scaleUp_25to75 = [];
		shapeForInterpolation_scaleDn_25to75 = [];
		shapeForInterpolation_smearUp_25to75 = [];
		shapeForInterpolation_smearDn_25to75 = [];

		sigMassesForInterpolation_50to100 = [];
		shapeForInterpolation_central_50to100 = [];
		shapeForInterpolation_scaleUp_50to100 = [];
		shapeForInterpolation_scaleDn_50to100 = [];
		shapeForInterpolation_smearUp_50to100 = [];
		shapeForInterpolation_smearDn_50to100 = [];


		sigMassesForInterpolation_75to125 = [];
		shapeForInterpolation_central_75to125 = [];
		shapeForInterpolation_scaleUp_75to125 = [];
		shapeForInterpolation_scaleDn_75to125 = [];
		shapeForInterpolation_smearUp_75to125 = [];
		shapeForInterpolation_smearDn_75to125 = [];
		...
		for pFunc in iFuncs:
			...
			if iShift and ("wqq" in process or "zqq" in process):
				...
				if mass > 0 and process != "wqq" and process != "zqq":
					...
					if mass < 80 and mass > 20:
						sigMassesForInterpolation_25to75.append(mass);     
						shapeForInterpolation_central_25to75.append(hmatched_new_central) 
						shapeForInterpolation_scaleUp_25to75.append(hmatchedsys_shift[0]) 
						shapeForInterpolation_scaleDn_25to75.append(hmatchedsys_shift[1])  
						shapeForInterpolation_smearUp_25to75.append(hmatchedsys_smear[0])  
						shapeForInterpolation_smearDn_25to75.append(hmatchedsys_smear[1]) 
					if mass < 105 and mass > 45:
						sigMassesForInterpolation_50to100.append(mass);     
						shapeForInterpolation_central_50to100.append(hmatched_new_central) 
						shapeForInterpolation_scaleUp_50to100.append(hmatchedsys_shift[0]) 
						shapeForInterpolation_scaleDn_50to100.append(hmatchedsys_shift[1])  
						shapeForInterpolation_smearUp_50to100.append(hmatchedsys_smear[0])  
						shapeForInterpolation_smearDn_50to100.append(hmatchedsys_smear[1]) 
					if mass < 130 and mass > 60:
						sigMassesForInterpolation_75to125.append(mass);     
						shapeForInterpolation_central_75to125.append(hmatched_new_central) 
						shapeForInterpolation_scaleUp_75to125.append(hmatchedsys_shift[0]) 
						shapeForInterpolation_scaleDn_75to125.append(hmatchedsys_shift[1])  
						shapeForInterpolation_smearUp_75to125.append(hmatchedsys_smear[0])  
						shapeForInterpolation_smearDn_75to125.append(hmatchedsys_smear[1])
				print "++++++++++++++++"
				print "++++++++++++++++"
				print "++++++++++++++++"
				print len(shapeForInterpolation_central_10to25) 
				print len(shapeForInterpolation_central_25to75) 
				print len(shapeForInterpolation_central_50to100) 
				print len(shapeForInterpolation_central_75to125) 
				print "++++++++++++++++"
				print "++++++++++++++++"
				print "++++++++++++++++"
		...
		morphedHistContainer_central_25to75 = hist(sigMassesForInterpolation_25to75,shapeForInterpolation_central_25to75);
		morphedHistContainer_scaleUp_25to75 = hist(sigMassesForInterpolation_25to75,shapeForInterpolation_scaleUp_25to75);
		morphedHistContainer_scaleDn_25to75 = hist(sigMassesForInterpolation_25to75,shapeForInterpolation_scaleDn_25to75);
		morphedHistContainer_smearUp_25to75 = hist(sigMassesForInterpolation_25to75,shapeForInterpolation_smearUp_25to75);
		morphedHistContainer_smearDn_25to75 = hist(sigMassesForInterpolation_25to75,shapeForInterpolation_smearDn_25to75);

		morphedHistContainer_central_50to100 = hist(sigMassesForInterpolation_50to100,shapeForInterpolation_central_50to100);
		morphedHistContainer_scaleUp_50to100 = hist(sigMassesForInterpolation_50to100,shapeForInterpolation_scaleUp_50to100);
		morphedHistContainer_scaleDn_50to100 = hist(sigMassesForInterpolation_50to100,shapeForInterpolation_scaleDn_50to100);
		morphedHistContainer_smearUp_50to100 = hist(sigMassesForInterpolation_50to100,shapeForInterpolation_smearUp_50to100);
		morphedHistContainer_smearDn_50to100 = hist(sigMassesForInterpolation_50to100,shapeForInterpolation_smearDn_50to100);


		morphedHistContainer_central_75to125 = hist(sigMassesForInterpolation_75to125,shapeForInterpolation_central_75to125);
		morphedHistContainer_scaleUp_75to125 = hist(sigMassesForInterpolation_75to125,shapeForInterpolation_scaleUp_75to125);
		morphedHistContainer_scaleDn_75to125 = hist(sigMassesForInterpolation_75to125,shapeForInterpolation_scaleDn_75to125);
		morphedHistContainer_smearUp_75to125 = hist(sigMassesForInterpolation_75to125,shapeForInterpolation_smearUp_75to125);
		morphedHistContainer_smearDn_75to125 = hist(sigMassesForInterpolation_75to125,shapeForInterpolation_smearDn_75to125);
		#interpolatedMasses = [40.]
		interpolatedMasses = [15.,20.,30.,35.,40.,45.,55.,60.0,65.,70.,80.,85.,90.0,95.,105.,110.0,115.,120.]
		...
		for m in interpolatedMasses:
			if m < 25.:
				...
				print "-------------------------------------------------------"
				print "       interpolating mass " + str(m)
				htmp_central = morphedHistContainer_central_25to75.morph(m);
				htmp_scaleUp = morphedHistContainer_scaleUp_25to75.morph(m);
				htmp_scaleDn = morphedHistContainer_scaleDn_25to75.morph(m);
				htmp_smearUp = morphedHistContainer_smearUp_25to75.morph(m);
				htmp_smearDn = morphedHistContainer_smearDn_25to75.morph(m);
				htmp_central.SetName("zqq%i_%s" % (int(m),iCat));
				self.InterpoMasses.append([htmp_central, iCat, int(m)])
				htmp_scaleUp.SetName("zqq%i_%s_scaleUp" % (int(m),iCat)); 
				htmp_scaleDn.SetName("zqq%i_%s_scaleDown" % (int(m),iCat));
				...
				print "-------------------------------------------------------"
				print "       interpolating mass " + str(m)
				htmp_central = morphedHistContainer_central_50to100.morph(m);
				htmp_scaleUp = morphedHistContainer_scaleUp_50to100.morph(m);
				htmp_scaleDn = morphedHistContainer_scaleDn_50to100.morph(m);
				htmp_smearUp = morphedHistContainer_smearUp_50to100.morph(m);
				htmp_smearDn = morphedHistContainer_smearDn_50to100.morph(m);
				htmp_central.SetName("zqq%i_%s" % (int(m),iCat));
				self.InterpoMasses.append([htmp_central, iCat, int(m)])
				htmp_scaleUp.SetName("zqq%i_%s_scaleUp" % (int(m),iCat)); 
				htmp_scaleDn.SetName("zqq%i_%s_scaleDown" % (int(m),iCat));
				...
				print "-------------------------------------------------------"
				print "       interpolating mass " + str(m)
				htmp_central = morphedHistContainer_central_75to125.morph(m);
				htmp_scaleUp = morphedHistContainer_scaleUp_75to125.morph(m);
				htmp_scaleDn = morphedHistContainer_scaleDn_75to125.morph(m);
				htmp_smearUp = morphedHistContainer_smearUp_75to125.morph(m);
				htmp_smearDn = morphedHistContainer_smearDn_75to125.morph(m);
				htmp_central.SetName("zqq%i_%s" % (int(m),iCat));
				self.InterpoMasses.append([htmp_central, iCat, int(m)])
				htmp_scaleUp.SetName("zqq%i_%s_scaleUp" % (int(m),iCat)); 
				htmp_scaleDn.SetName("zqq%i_%s_scaleDown" % (int(m),iCat));
...
if __name__ == '__main__':
	...
	parser.add_option('--input', dest='input', default = 'ZGammaFermiSRTemplates.root',help='directory with data', metavar='idir')
