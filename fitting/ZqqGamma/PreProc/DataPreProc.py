...
class MicroTree:
	...
	def FillMicroTree(self):
		...
		for j in range(0, self.n): # Here is where we loop over all events.
			...
			if T.vpho0_pt > 200 and T.vpho0_mva > 0.2 and T.AK8Puppijet0_pt > 200. and T.AK8Puppijet0_msd > 0.:
				...
				if T.pfmet < 100. and maxbinevent < 0.5426 and T.neleLoose == 0 and isolated and T.AK8Puppijet0_isTightVJet ==1 and T.nmuLoose == 0 and T.ntau==0 and Jet.DeltaR(Photon) > 2.2 :
					whichtree = self.tree
...
if __name__ == '__main__':
	...
	V10 = MicroTree("ZQQpG_10", "VectorDiJet1Gammamva_M10_q.root", False)
	V25 = MicroTree("ZQQpG_25", "VectorDiJet1Gammamva_M25_q.root", False)
	V50 = MicroTree("ZQQpG_50", "VectorDiJet1Gammamva_M50_q.root", False)
	V75 = MicroTree("ZQQpG_75", "VectorDiJet1Gammamva_M75_q.root", False)
	V100 = MicroTree("ZQQpG_100", "VectorDiJet1Gammamva_M100_q.root", False)
	V125 = MicroTree("ZQQpG_125", "VectorDiJet1Gammamva_M125_q.root", False)
	V20 = MicroTree("ZQQpG_200", "VectorDiJet1Gammamva_M200_q.root", False)

	TT = MicroTree("ttbar", "TTmvaEVv3_q.root", False)
	QCD = MicroTree("qcd", "QCDmvaEVwithExtv3_q.root", False)
	GJET = MicroTree("gjets", "GJetsmvaEVv3withExt.root", False)

	Data = MicroTree("data", "SinglePhotonmvaEV12av3_q.root", True)
